...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            $export_url = url('/export/expenses');
            if (!empty($query_params)) {
                $export_url .= '?' . http_build_query($query_params);
            }
            $jsondata['dom_attributes'][] = [
                'selector' => '#export-expenses-button',
                'attr' => 'data-url',
                'value' => $export_url,
            ];
            ...
            return view('pages/expenses/wrapper', compact('page', 'expenses', 'stats', 'categories', 'projects', 'purchases'))->render();
        }
