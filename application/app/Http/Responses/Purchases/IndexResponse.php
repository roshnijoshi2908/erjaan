...
class IndexResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if (request('source') == 'ext' || request('action') == 'search' || request()->ajax()) {
            ...
            case 'load':
                $template = 'pages/purchase/components/table/ajax';
                $dom_container = '#purchases-td-container';
                $dom_action = 'append';
                break;
            ...
            case 'search':
                $template = 'pages/purchase/components/table/table';
                $dom_container = '#purchases-table-wrapper';
            ...
            if ($purchase->currentPage() < $purchase->lastPage()) {
                $url = loadMoreButtonUrl($purchase->currentPage() + 1, request('source'));
            ...
            $html = view($template, compact('page', 'purchase'))->render();
            ...
            //standard view
            $page['url'] = loadMoreButtonUrl($purchase->currentPage() + 1, request('source'));
            $page['loading_target'] = 'purchase-td-container';
            $page['visibility_show_load_more'] = ($purchase->currentPage() < $purchase->lastPage()) ? true : false;
            return view('pages/purchase/wrapper', compact('page','purchase', 'supplier', 'project', 'client'))->render();
        }
