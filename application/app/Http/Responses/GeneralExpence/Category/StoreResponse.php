...
class StoreResponse implements Responsable {
    ...
    public function toResponse($request) {
        ...
        if ($count == 1) {
            $html = view('pages/generalexpence/generalexpencecategory/components/table/table', compact('generalexpencecat'))->render();
            $jsondata['dom_html'][] = array(
                'selector' => '#expencecat-table-wrapper-table-wrapper',
                'action' => 'replace',
                'value' => $html);
        } else {
            ...
            $jsondata['dom_html'][] = array(
                'selector' => '#expencecat-td-container',
                'action' => 'prepend',
                'value' => $html);
        }
