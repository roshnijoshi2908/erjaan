...
class Visibility
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        $this->viewReports();
        $this->viewSuppliers();
        $this->viewProcurement();
        $this->viewGeneralExpence();
    ...
    /**
     * visibility of the Suppliers features [both]
     */
    public function viewSuppliers()
    {
        
        if (auth()->user()->role->role_supplier >= 1) {
            config(['visibility.modules.supplier' => true]);
        }
        
        if (auth()->user()->role->role_supplier_category >= 1) {
            config(['visibility.modules.supplier_category' => true]);
        }
    }
    
    /**
     * visibility of the Procurement features [both]
     */
    public function viewProcurement()
    {
        
        if (auth()->user()->role->role_purchases >= 1) {
            config(['visibility.modules.purchases' => true]);
        }
        
        if (auth()->user()->role->role_purchase_gm_approval >= 1) {
            config(['visibility.modules.purchase_gm_approval' => true]);
        }
        
        if (auth()->user()->role->role_purchase_accountant >= 1) {
            config(['visibility.modules.purchase_accountant' => true]);
        }
    }
    
    /**
     * visibility of the General expence features [both]
     */
    public function viewGeneralExpence()
    {
        
        if (auth()->user()->role->role_generalexpence >= 1) {
            config(['visibility.modules.generalexpence' => true]);
        }
        
        if (auth()->user()->role->role_generalexpence_gm_approval >= 1) {
            config(['visibility.modules.generalexpence_gm_approval' => true]);
        }
        
        if (auth()->user()->role->role_generalexpence_accountant >= 1) {
            config(['visibility.modules.generalexpence_accountant' => true]);
        }
        
        if (auth()->user()->role->role_generalexpence_category >= 1) {
            config(['visibility.modules.generalexpence_category' => true]);
        }
    }
