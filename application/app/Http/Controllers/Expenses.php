...
class Expenses extends Controller {
    ...
    /**
     * The purhases repository instance.
     */
    protected $purchasesrepo;
     ...
     */
    public function index(ProjectRepository $projectrepo, CategoryRepository $categoryrepo, PurchasesRepository $purchasesrepo) {
        ...
        //get clients purchases list
        // if (config('visibility.filter_panel_clients_projects')) {
               $purchases = $purchasesrepo->search('', [
                'purchase_clientid' => request('filter_expense_clientid'),
     ...
     * @return \Illuminate\Http\Response
     */
    public function create(CategoryRepository $categoryrepo,  PurchasesRepository $purchasesrepo) {
       ...
       $purchases = $purchasesrepo->search('', [
            'purchase_clientid' => request('expense_clientid'),
            'purchase_status' => 'Approved'
       ]);
    ...
    public function store(ExpenseStoreUpdate $request) {
        ...
        $expense_clientid = $request->expense_clientid;
        $totalPayments = Payment::where('payment_clientid', $expense_clientid)->sum('payment_amount');
        $totalExpenses = Expense::where('expense_clientid', $expense_clientid)->sum('expense_amount');
        $paymentLimit = $totalPayments * 70 / 100;
        $overAllExpenses = $totalExpenses + (float)$request->expense_amount;
        if ($overAllExpenses > $paymentLimit) {
            abort(409, __('lang.total_expense_limit_exceeded'));
        }
     ...
     * @return \Illuminate\Http\Response
     */
    public function edit(CategoryRepository $categoryrepo, $id,  PurchasesRepository $purchasesrepo) {
    ...
    public function update($id) {
        ...
        $validator = Validator::make(request()->all(), [
            ...
            'expense_categoryid' => [
                ...
                //       $categoryId = (int) request('expense_categoryid');
                     
                //         if (!\App\Models\Category::find($categoryId)) {
