...
class RoleRepository {
    ...
    public function create() {
        ...
        $role->role_dashboard = (in_array(request('role_dashboard'),$valid)) ? request('role_dashboard') : 0;
        $role->role_dashboard_scope = (request('role_dashboard_scope') == 'on') ? 'global' : 'own';
        
        $role->role_supplier = (in_array(request('role_supplier'),$valid)) ? request('role_supplier') : 0;
        $role->role_supplier_scope = (request('role_supplier_scope') == 'on') ? 'global' : 'own';

        $role->role_purchases = (in_array(request('role_purchases'),$valid)) ? request('role_purchases') : 0;
        $role->role_purchases_scope = (request('role_purchases_scope') == 'on') ? 'global' : 'own';
        
        $role->role_purchase_gm_approval = (in_array(request('role_purchase_gm_approval'),$valid)) ? request('role_purchase_gm_approval') : 0;
        $role->role_purchase_gm_approval_scope = (request('role_purchase_gm_approval_scope') == 'on') ? 'global' : 'own';

        $role->role_purchase_accountant = (in_array(request('role_purchase_accountant'),$valid)) ? request('role_purchase_accountant') : 0;
        $role->role_purchase_accountant_scope = (request('role_purchase_accountant_scope') == 'on') ? 'global' : 'own';
        
        $role->role_generalexpence = (in_array(request('role_generalexpence'),$valid)) ? request('role_generalexpence') : 0;
        $role->role_generalexpence_scope = (request('role_generalexpence_scope') == 'on') ? 'global' : 'own';
        
         $role->role_generalexpence_accountant = (in_array(request('role_generalexpence_accountant'),$valid)) ? request('role_generalexpence_accountant') : 0;
        $role->role_generalexpence_accountant_scope = (request('role_generalexpence_accountant_scope') == 'on') ? 'global' : 'own';
        
        $role->role_generalexpence_gm_approval = (in_array(request('role_generalexpence_gm_approval'),$valid)) ? request('role_generalexpence_gm_approval') : 0;
        $role->role_generalexpence_gm_approval_scope = (request('role_generalexpence_gm_approval_scope') == 'on') ? 'global' : 'own';

        $role->role_generalexpence_category = (in_array(request('role_generalexpence_category'),$valid)) ? request('role_generalexpence_category') : 0;
        $role->role_generalexpence_category_scope = (request('role_generalexpence_category_scope') == 'on') ? 'global' : 'own';
    ...
    public function update($id) {
        ...
        $role->role_supplier_category = (in_array(request('role_supplier_category'),$valid)) ? request('role_supplier_category') : 0;
        $role->role_supplier_category_scope = (request('role_supplier_category_scope') == 'on') ? 'global' : 'own';
