...
class LeadRepository {
    ...
    public function search($id = '', $data = []) {
        ...
        if(isset($data['group_by_telephone']) && $data['group_by_telephone']) {
            
            $leads->selectRaw('SUM(CASE WHEN leads.lead_converted = "yes" THEN leads.lead_value ELSE 0 END) AS total_converted_lead_value');

            $leads->selectRaw('SUM(CASE WHEN leads.lead_converted = "no" THEN leads.lead_value ELSE 0 END) AS total_non_converted_lead_value');
            $leads->groupBy('leads.lead_phone');
        }
        ...
        if (request()->has('filter_phone')) {
            $leads->where('lead_phone', request('filter_phone'));
        }
