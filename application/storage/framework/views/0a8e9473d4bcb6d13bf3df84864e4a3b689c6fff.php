    ...
    <div class="x-section">
        ...
        <form id="action-taken-form">
            <span class="list-checkboxes w-px-20">
                <input type="checkbox" id="action-taken" name="action-taken" class="listcheckbox-all filled-in chk-col-light-blue"
                    <?php echo e($lead->actionTaken === 1 ? 'checked' : ''); ?>

                    <?php echo e($lead->actionTaken === null ? 'checked' : ''); ?> >
                <label for="action-taken"><?php echo app('translator')->get('lang.immediate_action'); ?></label>
            </span>
             <button type="button" class="btn btn-danger  btn-xs js-ajax-ux-request x-submit-button disable-on-click"
                id="action-taken-submit-button" data-url="/leads/<?php echo e($lead->lead_id); ?>/add-action-taken" data-type="form" data-ajax-type="post"
                data-form-id="action-taken-form">
                 <?php echo e(cleanLang(__('lang.save'))); ?>
    ...
    <script>
         $(document).on("click", "#action-taken-submit-button", function () {
             if (!$("#action-taken").is(":checked")) {
               
                $("#card-reminder-create-button").click();
            }
        });
    </script><?php /**PATH /home2/lhxwbjte/erjaan.com/application/resources/views/pages/lead/rightpanel.blade.php ENDPATH**/ ?>