<div class="header-cover" id="hero-header-cover" {!! getDocumentHeroImage($document->doc_hero_direcory ?? '',
    ...
    <div class="doc-hero-header {{ documentEditingModeCheck1($payload['mode'] ?? '') }}"
        ...
        <!--main heading-->
        <div class="main-heading" data-block-src="block_sub_heading_1" {!! _clean(getFontColor($document->
            doc_heading_color ?? '')) !!}>{{ $document->doc_heading }}</div>

        <!--document title-->
        <div class="main-title" {!! getFontColor($document->doc_title_color ?? '') !!}>{{ $document->doc_title }}
        </div>
    </div>
</div>