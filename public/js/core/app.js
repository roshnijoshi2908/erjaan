...
$(document).ready(function () {
    ...
    $(document).ready(function() {
        $(document).on('change','.is_this_sales_person',function(){
            if($(this).is('checked')){
                $('#target_revenue_div').show()
            }else{
                $('#target_revenue_div').hide()
            }
    ...
    $(document).ready(function(){
        //dashboard chart
     if ($("#dashboardWidget").length) {
        var chart = c3.generate({
            bindto: '#dashboardWidget',
            data: {
    ...
    $(document).ready(function(){
        if ($("#dashboardCategoryBarChart").length) {
            c3.generate({
                bindto: '#dashboardCategoryBarChart',
                data: {
                    columns: [NX.admin_home_c3_category_data],
                    type: 'bar'
                },
                axis: {
                    x: {
                        type: 'category',
                        categories: NX.admin_home_c3_category_value
                    }
                },
                bar: {
                    width: {
    ...
    $(document).ready(function(){
     ...
     if ($("#dashboardLeadSourceBarChart").length) {
        var chart = c3.generate({
            bindto: '#dashboardLeadSourceBarChart',
            data: {
                columns: [NX.admin_home_c3_lead_source_data],
                type: 'bar'
            },
            axis: {
                x: {
                    type: 'category',
                    categories: NX.admin_home_c3_lead_source_value
                }
            },
            bar: {
                width: {
    ...
    $(document).ready(function(){
     ...
     if ($("#dashboardLeadsConvertedBarChart").length) {
        c3.generate({
            bindto: '#dashboardLeadsConvertedBarChart',
            data: {
                columns: [NX.admin_home_c3_leads_converted_data],
                type: 'bar'
            },
            axis: {
                x: {
                    type: 'category',
                    categories: NX.admin_home_c3_leads_converted_value
                }
            },
            bar: {
                width: {
...
$(document).ready(function(){
         if ($("#gaugeWidget").length) {
            var chart = c3.generate({
                bindto: '#gaugeWidget',
                data: {
                    columns: [
                        ['data', NX.admin_home_c3_gauge_ratio]
                    ],
                    type: 'gauge'
                },
                color: {
        ...
        if ($("#gaugeWidget1").length) {
            var chart = c3.generate({
                bindto: '#gaugeWidget1',
                data: {
                    columns: [
                        ['data', NX.admin_home_c3_gauge_qulified_ratio]
                    ],
                    type: 'gauge'
                },
                color: {
...
// direction update based on language
...
$(document).ready(() => {
    const isRtl = localStorage.getItem('rtl') ? localStorage.getItem('rtl') : false;
    if(isRtl == 2) {
        $('body').addClass('rtl');
        $('body').attr('dir', 'rtl');
        localStorage.setItem('rtl', 2);
    } else {
        $('body').removeClass('rtl');
        $('body').attr('dir', 'ltr');
        localStorage.setItem('rtl', 3);
    }
});

/**--------------------------------------------------------------------------------------
 * [Invoice - UPDATE INVOICE LOGO]
 * @description: validation for Invoice logo modal
 * -------------------------------------------------------------------------------------*/


    //upload avatar
    $("#fileupload_logo").dropzone({
        url: "invoices/invoice-logo",
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        maxFiles: 1,
        ...
        success: function (file, response) {
            ...
            var $preview = $(file.previewElement);
            //create a hidden form field for this file
            $preview.append('<input type="hidden" name="logo"  value="' + response.filename + '">');
        }
    });
